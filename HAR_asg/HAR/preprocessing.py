...
def add_features(df, COLNAMES_TRAIN = ['acc_z', 'acc_xy', 'gyro_x','gyro_y','gyro_z','label','id']):
    ...
    for variable in COLNAMES_TRAIN[0:5]:
        ...
        min_results = apply_to_window(data, len(set(df['window_id'])), variable, min)
        data = map_back_to_window(data, {key: min_results[key]for key in min_results}, 'min_'+variable)
        max_results = apply_to_window(data, len(set(df['window_id'])), variable, max)
...
def map_back_more_windows(data, result_dic, new_col_name, column_key='window_id'):
    datacopy = deepcopy(data)
    for index, name in enumerate(new_col_name):
      datacopy[name] = [result_dic.get(str(int(val)))[index] for val in datacopy[column_key]]
    return datacopy
